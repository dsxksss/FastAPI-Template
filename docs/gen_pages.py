...
from typing import Dict, List, Any, Optional, get_type_hints, get_args, get_origin, Union
...
try:
    from fastapi import FastAPI, Query, Body, Path as FastAPIPath
...
def get_model_fields(model: type[BaseModel]) -> List[Dict[str, Any]]:
    ...
    for field_name, field_info in model.model_fields.items():
        ...
        if hasattr(field_info, 'examples') and field_info.examples:
            example = field_info.examples[0] if isinstance(field_info.examples, list) else field_info.examples
                ...
                if hasattr(param.annotation, '__origin__'):
                    if param.annotation.__origin__ is Union:
                        ...
                        if len(args) == 2 and type(None) in args:
                            ...
                            param_info['type'] = str(non_none_type).replace('typing.', '').replace('builtins.', '')
                        else:
                            param_info['type'] = str(param.annotation).replace('typing.', '').replace('builtins.', '')
                    else:
                        param_info['type'] = str(param.annotation).replace('typing.', '').replace('builtins.', '')
                else:
                    # 简化类型名称
...
def generate_request_body_section(request_body: Dict[str, Any]) -> str:
    ...
    content = "### 请求体\n\n"
    content += f"**Content-Type**: `application/json`\n\n"
...
"""
    ...
    for route_data in routes:
        # 提取详细信息
        route_details = extract_route_details(route_data) if isinstance(route_data, APIRoute) else route_data
        ...
        method = list(route_details['methods'])[0] if route_details['methods'] else 'GET'
        
        # 构建cURL命令
        curl_cmd = f'curl -X {method} "http://localhost:8000{route_details["path"]}'
        
        # 添加查询参数示例
        query_params = [p for p in route_details.get('parameters', []) if p['in'] == 'query']
        if query_params:
            param_examples = []
        ...
        if method == 'GET':
            if query_params:
                ...
                content += "}\n\n"
                content += f'response = requests.get(\n'
            ...
            if route_details.get('request_body') and route_details['request_body'].get('model_fields'):
                content += "data = {\n"
                example_count = 0
                for field in route_details['request_body']['model_fields']:
                    ...
                    if field.get('example') is not None:
                        ...
                        if 'email' in field_name:
                            ...
                            if 'desc' in field_name or 'description' in field_name:
                                ...
                                content += f'    "{field["name"]}": "/api/v1/example",\n'
                            elif 'method' in field_name:
            ...
            content += f'response = requests.delete(\n'
            content += f'    "http://localhost:8000{route_details["path"]}"'
            if module_name != 'base':
                content += ',\n    headers=headers'
            content += '\n)\n'
...
def main():
    ...
    if app is not None:
        try:
            ...
            # 生成基础文档结构
            basic_modules = ['base', 'users', 'role', 'menu', 'files', 'dept', 'api', 'auditlog']
