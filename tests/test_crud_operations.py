...
class TestCRUDOperations:
    ...
    async def test_user_crud_full_cycle(self, async_client: AsyncClient, admin_token: str):
        """测试用户CRUD完整流程"""
    ...
    async def test_user_creation_validation(self, async_client: AsyncClient, admin_token: str):
        """测试用户创建数据验证"""
        headers = {"Authorization": f"Bearer {admin_token}"}
    ...
    async def test_duplicate_user_creation(self, async_client: AsyncClient, admin_token: str):
        """测试创建重复用户"""
        headers = {"Authorization": f"Bearer {admin_token}"}
    ...
    async def test_user_list_pagination(self, async_client: AsyncClient, admin_token: str):
        """测试用户列表分页"""
        headers = {"Authorization": f"Bearer {admin_token}"}
        
        # 测试分页参数
        response = await async_client.get(
            "/api/v1/users/list?page=1&page_size=5",
            headers=headers
        )
    ...
    async def test_user_search_functionality(self, async_client: AsyncClient, admin_token: str):
        """测试用户搜索功能"""
        headers = {"Authorization": f"Bearer {admin_token}"}
        ...
        search_response = await async_client.get(
            "/api/v1/users/list?username=search_test",
            headers=headers
        )
    ...
    async def test_nonexistent_user_operations(self, async_client: AsyncClient, admin_token: str):
        """测试对不存在用户的操作"""
        headers = {"Authorization": f"Bearer {admin_token}"}
        ...
        # 测试删除不存在的用户
        delete_response = await async_client.delete(
            f"/api/v1/users/delete/{nonexistent_id}",
            headers=headers
        )
