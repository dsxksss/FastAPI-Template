...
class TestPermissions:
    ...
    async def test_superuser_required_access_with_admin(self, async_client: AsyncClient, admin_token: str):
        """测试管理员访问需要超级用户权限的端点"""
        # 测试用户列表接口（通常需要管理员权限）
    ...
    async def test_superuser_required_access_with_normal_user(self, async_client: AsyncClient, normal_user_token: str):
        """测试普通用户访问需要超级用户权限的端点"""
        # 普通用户尝试访问管理员接口
        response = await async_client.get(
            "/api/v1/users/list", 
    ...
    async def test_authenticated_access_with_valid_token(self, async_client: AsyncClient, normal_user_token: str):
        """测试普通认证用户访问需要认证的端点"""
        response = await async_client.get(
    ...
    async def test_user_crud_permissions(self, async_client: AsyncClient, admin_token: str, normal_user_token: str):
        """测试用户CRUD操作权限"""
        # 测试创建用户（通常需要管理员权限）
        user_data = {
